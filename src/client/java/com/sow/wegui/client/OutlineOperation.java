...
/**
 * //outline (//faces) 的直接 API 实现。
 */
public final class OutlineOperation implements WeOperation {
    @Override
    public boolean execute(Minecraft mc, WeCommandUsage usage, List<String> values) {
        ...
        try {
            ...
            return WeOperationHelper.edit(mc, ctx.player, ctx.world, es -> es.makeCuboidFaces(ctx.region, pattern));
        } catch (Exception e) {
