...
/**
 * /removebelow 的直接 API 实现。
 */
public final class RemoveBelowOperation implements WeOperation {
    @Override
    public boolean execute(Minecraft mc, WeCommandUsage usage, List<String> values) {
        ...
        try {
            ...
            return WeOperationHelper.edit(mc, ctx.player, ctx.world, es ->
                    es.removeBelow(ctx.session.getPlacementPosition(ctx.player), finalSize, finalHeight));
        } catch (Exception e) {
