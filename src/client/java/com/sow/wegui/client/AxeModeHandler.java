...
public final class AxeModeHandler {
    ...
    private static void pasteAtPreview(LocalPlayer player) {
        ...
        if (!WorldEditBridge.canUseDirectPaste()) {
            ...
            if (Configs.PastePreview.PASTE_REPLACE_AIR_ONLY.getBooleanValue()) {
                player.sendOverlayMessage(Component.translatable("wegui.message.server_paste_air_only_unsupported").withStyle(ChatFormatting.RED));
                return;
            }
            ...
            resetPastePreviewOffset();
            player.sendSystemMessage(Component.translatable("wegui.message.server_paste_sent", plan.commands().size(), plan.undoSteps()).withStyle(ChatFormatting.GREEN));
            return;
        }
    ...
    private static void warnServerUnlockOnce(LocalPlayer player) {
        ...
        player.sendSystemMessage(Component.translatable("wegui.message.server_unlock_warning").withStyle(ChatFormatting.YELLOW));
    }
