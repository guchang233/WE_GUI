 ...
 * //fill 的直接 API 实现。
 */
public final class FillOperation implements WeOperation {
    @Override
    public boolean execute(Minecraft mc, WeCommandUsage usage, List<String> values) {
        ...
        try {
            ...
            int finalDepth = Math.max(1, depth);
            double finalRadius = Math.max(0, radius);
            return WeOperationHelper.edit(mc, ctx.player, ctx.world, es ->
                    es.fillXZ(ctx.session.getPlacementPosition(ctx.player), pattern, finalRadius, finalDepth, false));
        } catch (Exception e) {
