 ...
 * //cone 的直接 API 实现。
 */
public final class ConeOperation implements WeOperation {
    @Override
    public boolean execute(Minecraft mc, WeCommandUsage usage, List<String> values) {
        if (values.size() < 3 || values.get(0).isBlank() || values.get(1).isBlank() || values.get(2).isBlank()) {
            error(mc, "请指定图案、半径和高度");
            return false;
        }
        ...
        try {
            ...
            return WeOperationHelper.edit(mc, player, world,
                    es -> es.makeCone(finalPos, pattern, finalRadius, finalRadius, finalHeight, true, 1.0));
        } catch (Exception e) {
